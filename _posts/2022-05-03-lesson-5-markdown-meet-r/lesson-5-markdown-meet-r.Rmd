...
```{r color_scheme, eval=FALSE, echo=FALSE, fig.height=3}
...
```{r, echo=FALSE, eval=TRUE}
knitr::include_graphics("files/chunk.png")
```
...
Pretty please pay attention to the structure of the chunk. Three back ticks ( **\`\`\`** ), followed by open curly bracket ( **\{** ), the language name (**bash**), closed curly bracket ( **\}** ), **\<return\>**, the command you want to run, **\<return\>** again, and finally three back ticks ( **\`\`\`** ). If you fail to follow this format, RStudio will not be very nice. 
...
Suppose I want to generate some quick summary statistics for one of the built-in data sets using the `r autolink("base::summary()")` function. In my R Markdown document I would code the chunk like so.  
...
Adding a chuck name is *optional* but my advice is to always add a name. The main reason is that it is easier to track down problems if each chunk has a name---especially in large documents or websites.  Keep the name simple. If you follow the same rules I described in the post on  [naming things](../../posts/2022-04-21-naming-things/index.html), you should be OK. Bad chunk names can cause errors when a document is rendered.
...
```{r summary1, echo=TRUE, eval=TRUE}
base::summary(trees)
```
...
````
```{r summary, echo=TRUE, eval=FALSE}`r ''`
base::summary(trees)
```
````
...
```{r summary2, echo=TRUE, eval=FALSE}
base::summary(trees)
```
...
-   Option 1 `r fa(name = "cog", fill = "grey")` **Modify Chunk Options** ,  is a drop down menu that gives you an alternative way to set code chunk options. 
...
-   Option 3 `r fa(name = "play", fill = "green")`  will **Run the Current Chunk**. Incidentally, if you do not see these options it means something is wrong with the chunk formatting. 
